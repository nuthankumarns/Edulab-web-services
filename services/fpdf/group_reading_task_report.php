...
//print_r($data1);
		...
		$task=$_REQUEST['task'];
//www.tritonetech.com/php_uploads/porsche/webservice/fpdf/group_reading_task_report.php?school_id=&level_id=&class_id=&module_id=
...
$pdf->Cell( 0, 15, 'Group Reading Task Reports', 0, 0, 'C' );
$pdf->SetTextColor( $textColour[0], $textColour[1], $textColour[2] );
...
LEFT JOIN jos_porsche_group AS c ON b.group_id = c.group_id AND c.task='group_read'
AND c.module_id = '$module_id'
WHERE a.school_id = '$school_id'
AND a.level_id = '$level_id'
AND a.class_id = '$class_id'
AND a.ability = '$ability'");
$a="SELECT a.id ,a.name,  c.id AS group_content_id
FROM jos_porsche_student AS a
LEFT JOIN jos_porsche_group_members AS b ON a.id = b.student_id
LEFT JOIN jos_porsche_group AS c ON b.group_id = c.group_id AND c.task='group_read'
AND c.module_id = '$module_id'
...
//echo $students[$i]."<br/>";
	...
	$q3=$_DB->Query("SELECT count( id ) AS unanswered_questions
		FROM jos_porsche_group_reading_task
		WHERE group_content_id = '$group_content_id'
		AND q_s_id = '$students[$i]' AND filename=''");
	...
	$q5=$_DB->Query("SELECT group_reading 
	...
	$data[$i][4]=$_DB->GetResultValue($q5,0,'group_reading');
